...
class WebSettingController extends Controller
{
    ...
    public function putGeneralSetting(Request $request, $id)
    {
        $this->validate($request,[
           'title' => 'required',
            'images' => 'mimes:png|dimensions:max_width=225,max_height=65'
        ]);
        try {
    ...
    {
        $data['site_title'] = $this->site_title;
        $data['page_title'] = "Create Api Setting";
        return view('websetting.api-create',$data);
    }
    ...
    public function showApiCreate()
    {
        $data['site_title'] = $this->site_title;
        $data['page_title'] = "Show All API's";
        $data['menu'] = Api::all();
        return view('websetting.api-show',$data);
    }
    public function editApiCreate($id)
    {
        $data['site_title'] = $this->site_title;
        $data['page_title'] = "Edit API's";
        $data['menu'] = Api::findorFail($id);
        return view('websetting.api-edit',$data);
    }
    public function updateApiCreate(Request $request,$id)
    {
        $menus = Api::findOrFail($id);
        $this->validate($request,[
            'name' =>'required|unique:menus,name,'.$menus->id,
            'description' => 'required'
        ]);
    ...
    public function deleteApiCreate($id)
    {
        try {
            $menu = Api::findOrFail($id);
            $menu->delete();
            session()->flash('message', 'Menu Deleted Successfully.');
            Session::flash('type', 'success');
            return redirect()->back();
